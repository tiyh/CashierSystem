...
public class TradeServiceImpl  implements TradeService {
    ...
    public void test_monitor_schedule_logic() {
        // 启动交易保障线程
        HbRunner demoRunner = new HbRunner(monitorService);
    ...
    public CommonResult tradeQuery(Long orderId) {
        ...
        switch (result.getTradeStatus()) {
            ...
            case FAILED:
                ...
                return new CommonResult().failed("查询返回该订单支付失败或被关闭!!!");

            case UNKNOWN:
                log.error("系统异常，订单支付状态未知!!!");
                return new CommonResult().failed("系统异常，订单支付状态未知!!!");

            default:
                log.error("不支持的交易状态，交易返回异常!!!");
                return new CommonResult().failed("不支持的交易状态，交易返回异常!!!");
        }
    }
    ...
    public CommonResult tradeRefund(Long id,String storeId,String refundReason) {
        ...
        String refundAmount = order.getPayAmount().toPlainString();
        ...
        switch (result.getTradeStatus()) {
            ...
            case FAILED:
                log.error("支付宝退款失败!!!");
                return new CommonResult().failed("支付宝退款失败!!!");
            case UNKNOWN:
                log.error("系统异常，订单退款状态未知!!!");
                return new CommonResult().failed("系统异常，订单退款状态未知!!!");
            default:
                log.error("不支持的交易状态，交易返回异常!!!");
                return new CommonResult().failed("不支持的交易状态，交易返回异常!!!");
        }
    ...
    public CommonResult tradePreCreate(Long orderId,String operatorId,String storeId) {
        ...
        switch (result.getTradeStatus()) {
            ...
            case FAILED:
                log.error("支付宝预下单失败!!!");
                return new CommonResult().failed("支付宝预下单失败!!!");

            case UNKNOWN:
                log.error("系统异常，预下单状态未知!!!");
                return new CommonResult().failed("系统异常，预下单状态未知!!!");
            default:
                log.error("不支持的交易状态，交易返回异常!!!");
                return new CommonResult().failed("不支持的交易状态，交易返回异常!!!");
        }
